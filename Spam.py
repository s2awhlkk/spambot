...
def start(message):
    ...
    if user_id in allowed_users:
        ...
        bot.reply_to(message, 'اهلا بك في بوت الرفع الخارجي المطورين @Sw_Ww , شرح البوت تابعه قبل لا تبدي ', reply_markup=keyboard)
    else:
        bot.reply_to(message, 'انت غير مشترك في البوت للاشتراك في البوت : @Sw_Ww ')
...
def show_admin_commands(message):
    if str(message.from_user.id) == '124182145':  
